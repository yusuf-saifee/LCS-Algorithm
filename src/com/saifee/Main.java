...
public class Main {
    ...
    public static String[] LCS(String[] sourceWords, String [] targetWords, int m, int n) {
        ...
        for(int i=0; i<=m; i++) {
            for(int j=0; j<=n; j++) {
                ...
                else if (sourceWords[i-1].equalsIgnoreCase(targetWords[j-1])) {
        ...
        String[] lcsString = new String[index];
        /*
        ...
        while (k > 0 && l > 0){
             ...
             if(currentVal != lCSTable[k-1][l] && currentVal != lCSTable[k][l-1]){
                 lcsString[index-1] = sourceWords[k-1]; // Table indices starts with 1 but source indices starts with 0.
                 index --;
                 k--;
                 l--;
             }
        ...
        while (k > 0 && l > 0){
            if(sourceWords[k-1].equalsIgnoreCase(targetWords[l-1])){
                lcsString[index-1] = sourceWords[k-1];
                index --;
                k--;
                l--;
            }
